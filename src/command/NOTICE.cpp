...
int Command::execute_NOTICE() {
	if (_message.get_parameters().size() < 2)
		return reply(ERR_NEEDMOREPARAMS("NOTICE"), 461);
	...
	for (size_t i = 0; i < targets.size(); i++) {
		...
		if (target_name[0] == '#') {
			...
			if (!channel)
				...
				channel->send_message(_client, CMD_NOTICE(target_name, message));
		} else {
			Client* target = _server->get_client(target_name);
			if (!target)
				reply(ERR_NOSUCHNICK(target_name), 401);
			else
				_client->send_to(*target, CMD_NOTICE(target_name, message));
		}
	}
	return 0;
}
